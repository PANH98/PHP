...
#Si no viene definina esta variable, se generará la página# 
        if(!isset($_GET['page'])){
            $pagina=1;
        }else{
            # Conversión de variable a entero#
            $pagina=(int) $_GET['page'];
            if($pagina<=1){
                $pagina=1;
            }
        }
#Se aplica la función de limpíeza de cadenas#
        $pagina=limpiar_cadena($pagina);
        #Se definen los numero de página#
        $url="index.php?vista=user_list&page=";
        $registros=15;
        #Busqueda mediante un termino#
        $busqueda="";
